...
export async function action( propes) {
  
  const formData = await propes.request.formData();
  console.log("request" , propes);

  return updateContact(propes.params.contactId, {
...
export default function Contact() {
  ...
  return (
    <div id="contact">
      ...
      <div>
        <h1>
          {contact.name || contact.last ? <>{contact.name}</> : <i>No Name</i>} <Favorite contact={contact} />
        </h1>
